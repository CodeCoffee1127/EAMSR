...
print("="*55)
print("场景 S2：POB 禁飞区约束验证（AirSimNH 城市环境）")
...
# 禁飞区参数（对应 AirSimNH 中建筑密集区）
NFZ_CENTER = (70, 50)
NFZ_RADIUS = 20  # 修改为 20m
...
ax.plot(original_route_x, original_route_y, color='#e74c3c', linewidth=2.5,
        ...
        label='Original Route (BW-Airspace FAIL)')

# 在穿越点标注红色 X（65,45）
crossing_point = (65, 45)
ax.scatter(*crossing_point, color='#e74c3c', s=350, marker='X',
           zorder=6, label='BW-Airspace Violation Point')
ax.annotate('BW-Airspace FAIL\n(NFZ Breach)',
            xy=crossing_point,
...
# (70,83): sqrt((70-70)^2 + (83-50)^2) = 33m ✓
refined_route_x = [0, 25, 45, 35, 70, 90]
refined_route_y = [0, 10, 25, 80, 83, 60]
...
# 绕飞安全裕度标注（最小距离点到 NFZ 边界的距离）
# (70,83) 到圆心距离 33m，减去半径 20m = 13m 裕度
# (35,80) 到圆心距离 46m，减去半径 20m = 26m 裕度
min_margin = 35  # 标注值（精化后路径保持 35m 安全距离）
ax.annotate(f'Min. margin to R-NJ-04: ~{min_margin}m',
            xy=(35, 80),
            xytext=(10, 95),
...
ax.set_title('AirSim-E2: BW-Airspace Refinement — Bypass vs. Direct Path Comparison\n'
...
out = r'D:\AirSim\UAV-PY\s2_airspace_v4.png'
...
print("[DONE] 场景 S2 完成 — 请立即对 AirSim 窗口截图 (Win+Shift+S)")
